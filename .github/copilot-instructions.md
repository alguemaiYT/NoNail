...
.venv/bin/pytest -q -k "test_rate_limit_detection"

# Lint
.venv/bin/ruff check nonail tests
```

The optional C++ accelerator (`fastpath.py`) is compiled by `setup.py` using `-O3 -march=native`. It has a pure-Python fallback — never required for tests.

## Architecture

NoNail is a provider-agnostic AI agent with two operating modes:
...
| `mcp_client.py` | `MCPClientManager`; connects to external servers; wraps tools as proxies |
...
3. `to_openai_schema()` is inherited from the ABC — don't override unless needed
